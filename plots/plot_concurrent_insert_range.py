...
def plot_rates(results_dir, output_dir, find_limits, spacing=[1, 50]):
    ...
    for s in initial_sizes:
        ...
        for r in range_lengths:
            p2 = 'range_length' + str(r)
            for u in update_ratios:
                p1 = 'update' + str(u) + '_'
                df = pd.read_csv(results_dir + subdir + p0 + p1 + p2 + '.csv')
                dfs.append(df)

    svg_name = 'insertion_rq_rates'
    # allocator_name = 'bump'
    allocator_name = 'slab'
    ...
    for s in range(len(initial_sizes)):
        ...
        df_offset = s * len(update_ratios) * len(range_lengths)
        x_axis = dfs[df_offset]['num_insertions'] + \
            dfs[df_offset]['num_range_query']
        ...
        for t in range(len(titles)):
            for u in range(len(update_ratios)):
                ...
                print_summary(str(initial_sizes[s]) + 'Mkeys_' + str(titles[t]) + 'RQ_' + str(update_ratios[u]) + 'alpha', dfs[df_offset + u + t * len(update_ratios)][column_names[1] + 'concurrent_ops'],
        ...
        fig.text(0.6, 0.78, textstr, fontsize=font_size)
...
if __name__ == "__main__":
    ...
    plot_rates(args.dir, args.output_dir, [
               args.min_find_throughput, args.max_find_throughput], [1, 100])

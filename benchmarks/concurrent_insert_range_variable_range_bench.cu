...
int main(int argc, char **argv) {
  ...
  std::string report_dir =
      output_dir + '/' + device_name + "/versioned_insert_range_variable_range/";
  std::filesystem::create_directories(report_dir);

  std::string filename = "rates_initial" + std::to_string(int(initial_tree_size / 1e6)) +
                         "M_update" + std::to_string(int(update_ratio * 100)) + "_num_ops" +
                         std::to_string(int(num_operations / 1e6)) + ".csv";
  ...
  if (!output_file_exist) {
    ...
    result_output << "average_range_length" << ',';

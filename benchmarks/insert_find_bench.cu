...
bench_rates bench_versioned_insertion_find(thrust::device_vector<KeyT>& d_keys,
  ...
  for (std::size_t exp = 0; exp < num_experiments; exp++) {
    ...
    if (validate_tree_structure) {
      std::cout << "Validating tree" << std::endl;
    ...
    if (validate_result) {
      ...
      utils::validate(ref_set, h_queries, d_results, to_value);
    }
  }
...
int main(int argc, char** argv) {
  ...
  bool validate_all      = get_arg_value<bool>(arguments, "validate").value_or(false);
  bool validate_result   = get_arg_value<bool>(arguments, "validate-result").value_or(validate_all);
  bool validate_tree     = get_arg_value<bool>(arguments, "validate-tree").value_or(validate_all);
  ...
  std::string report_dir = output_dir + '/' + device_name + "/blink_vs_versioned/";
  ...
  if (!output_file_exist) {
    ...
    result_output << "vblink_slab_in_place_insert" << ',';
    result_output << "vblink_slab_in_place_find" << ',';
    result_output << "vblink_bump_in_place_insert" << ',';
    result_output << "vblink_bump_in_place_find" << ',';

    result_output << "vblink_slab_out_of_place_insert" << ',';
    result_output << "vblink_slab_out_of_place_find" << ',';
    result_output << "vblink_bump_out_of_place_insert" << ',';
    result_output << "vblink_bump_out_of_place_find" << ',';
  ...
  {
    std::cout << "Benchmarking vblink_tree_slab_type in place" << std::endl;
    auto rates = bench_versioned_insertion_find<vblink_tree_slab_type, true>(d_keys,
                                                                             d_values,
                                                                             d_find_keys,
                                                                             d_results,
                                                                             h_keys,
  ...
  {
    std::cout << "Benchmarking vblink_tree_bump_type in place" << std::endl;
    auto rates = bench_versioned_insertion_find<vblink_tree_bump_type, true>(d_keys,
                                                                             d_values,
                                                                             d_find_keys,
                                                                             d_results,
                                                                             h_keys,
  ...
  {
    std::cout << "Benchmarking vblink_tree_slab_type out of place" << std::endl;
    auto rates = bench_versioned_insertion_find<vblink_tree_slab_type, true>(d_keys,
                                                                             d_values,
                                                                             d_find_keys,
                                                                             d_results,
  ...
  {
    std::cout << "Benchmarking vblink_tree_bump_type out of place" << std::endl;
    auto rates = bench_versioned_insertion_find<vblink_tree_bump_type, true>(d_keys,
                                                                             d_values,
                                                                             d_find_keys,
                                                                             d_results,
  ...
  {
    std::cout << "Benchmarking blink_tree_bump_type" << std::endl;

...
void batched_insertion_lookup(int argc, char** argv) {
  ...
  for (uint32_t batch_id = 0; batch_id < num_batches; batch_id++) {
    ...
    tree.find(d_keys.data().get(), d_batch_results[batch_id].data().get(), num_keys, find_stream);
    find_timers[batch_id]->stop_timer();
    // no concurrent operations here
    // todo: test concurrent finds
  ...
  if (validate) {
    for (uint32_t batch_id = 0; batch_id < num_batches; batch_id++) {
      ...
      auto cur_size    = last_offset - cur_offset;
      std::cout << "Validating Batch=" << batch_id << std::endl;
      utils::validate(
